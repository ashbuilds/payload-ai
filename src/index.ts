...
const payloadAiPlugin =
  ...
  (incomingConfig: Config): Config => {
    ...
    // Ensure admin provider is always registered for import-map generation
    {
    ...
    if (isActivated) {
      const Instructions = instructionsCollection(pluginConfig) as any
      // Inject editor schema to config, so that it can be accessed when /textarea endpoint will hit

      const lexicalSchema = lexicalJsonSchema(pluginConfig.editorConfig?.nodes || [])

      if (pluginConfig.debugging && Instructions.admin) {
