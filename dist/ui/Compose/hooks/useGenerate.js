...
export const useGenerate = ({ instructionId })=>{
    ...
    const { complete: completeRichText, completion: richTextCompletion, isLoading: loadingRichText, stop: stopRichText } = useCompletion({
    ...
    const { complete: completeText, completion: textCompletion, isLoading: loadingText, stop: stopText } = useCompletion({
        api: `${serverURL}${api}${PLUGIN_API_ENDPOINT_GENERATE}`,
        onError: (error)=>{
            toast.error(`Failed to generate: ${error.message}`);
    ...
    const streamRichText = useCallback(async ({ action = 'Compose', params })=>{
        const doc = getData();
